...
def get_camera_params(uv, pose, intrinsics):
    ...
    # print("ppc shape", pixel_points_cam.shape) [1, 2048, 4]
    # print("ppc", pixel_points_cam[:, :5, :]) [ 0.0079,  0.1019,  1.0000,  1.0000]

    # permute for batch matrix product
    pixel_points_cam = pixel_points_cam.permute(0, 2, 1)

    world_coords = torch.bmm(p, pixel_points_cam).permute(0, 2, 1)[:, :, :3]
    # print("wc", world_coords[:, :5, :]) [ 0.8511,  0.7473, -0.2879]
...
def get_sphere_intersection(cam_loc, ray_directions, r = 1.0):
    ...
    ray_cam_dot = torch.bmm(ray_directions, cam_loc).squeeze() # [1, 2048]
    under_sqrt = ray_cam_dot ** 2 - (cam_loc.norm(2,1) ** 2 - r ** 2)
    # cam_loc.norm(2, 1)：cam_loc到原点距离

    under_sqrt = under_sqrt.reshape(-1) # [2048]
    ...
    # tensor.clamp_min(x) 将tensor中所有小于x的元素置成x
